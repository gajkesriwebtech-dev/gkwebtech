...
export const ServicesPage: React.FC = () => {
  ...
  return (
    <div className="pt-32 pb-20 bg-bg-light dark:bg-gray-950 min-h-screen transition-colors">
      <Seo
        title={t('seo.services.title', 'Services | GK WebTech | GKWebTech')}
        description={t('seo.services.description', 'GK WebTech provides SEO, social media, Google Ads, Meta Ads, analytics, and WordPress services. GKWebTech helps businesses grow online.')}
        keywords={t('seo.services.keywords', 'services, SEO, social media, PPC, analytics, WordPress, creative design')}
        ...
        <div className="text-center mb-16 animate-fade-in-up">
          <div className="flex items-center justify-center gap-3 mb-3">
            ...
            <span className="text-sm font-semibold uppercase tracking-wider text-gray-600 dark:text-gray-400">
              {t("services_page.expertise", "Our Expertise")}
            </span>
            <span className="h-0.5 w-4 bg-secondary"></span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold text-text-dark dark:text-white mb-6">
            {t("services_page.all", "All")} <span className="text-secondary">{t("services_page.services", "Services")}</span>
          </h1>
          <p className="text-gray-700 dark:text-gray-400 max-w-2xl mx-auto text-lg leading-relaxed">
            {t("services_page.description", "We offer a comprehensive suite of digital marketing solutions designed to elevate your brand, drive traffic, and convert leads into loyal customers.")}
          </p>
        </div>
              ...
              <Link to={`/service/${service.id}`} className="block h-full">
                <TiltCard className="h-full relative z-0 hover:z-20 group">
                  <div className="bg-white dark:bg-gray-900 p-6 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] dark:shadow-none border border-gray-200 dark:border-gray-800 hover:border-secondary dark:hover:border-secondary transition-colors duration-300 cursor-pointer h-full relative overflow-hidden flex flex-col">
                    ...
                    <h3 className="text-2xl font-bold text-text-dark dark:text-white mb-4 line-clamp-1 group-hover:text-primary dark:group-hover:text-secondary transition-colors">
                      {t(`services.${service.id}.title`, service.title)}
                    </h3>
                    <p className="text-gray-700 dark:text-gray-400 mb-6 text-base leading-relaxed line-clamp-3 min-h-[4.5em] flex-grow">
