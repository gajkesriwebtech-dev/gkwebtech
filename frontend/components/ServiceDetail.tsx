  ...
  if (!service) {
    return (
      <div className="min-h-screen pt-32 flex flex-col items-center justify-center text-center px-4">
        <h2 className="text-4xl font-bold text-primary dark:text-white mb-4">{t("service.not_found", "Service Not Found")}</h2>
        <Link to="/" className="text-secondary hover:underline">{t("nav.home", "Return Home")}</Link>
      </div>
    );
  }
          ...
          <div className="absolute inset-0 z-20 flex items-center justify-center">
            <div className="container mx-auto px-4 text-center">
              <Link to="/#services" className="inline-flex items-center gap-2 text-white/80 hover:text-secondary mb-6 transition-colors">
                <ArrowLeft size={20} />
                <span>{t("service.back_services", "Back to Services")}</span>
              </Link>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="lg:col-span-2 space-y-8">
              <div className="bg-white dark:bg-gray-900 rounded-3xl p-8 md:p-12 shadow-xl border border-gray-100 dark:border-gray-800">
                <h2 className="text-2xl md:text-3xl font-bold text-primary dark:text-white mb-6">{t("service.overview", "Overview")}</h2>
                <p className="text-gray-600 dark:text-gray-300 leading-relaxed text-lg mb-8">
                  {t(`services.${service.id}.fullDescription`, service.fullDescription)}
                </p>

                <h3 className="text-xl font-bold text-primary dark:text-white mb-6">{t("service.features_title", "Key Features & Benefits")}</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(t(`services.${service.id}.features`, { returnObjects: true }) as string[] || service.features).map((feature, idx) => (
                ...
                <div className="grid grid-cols-2 h-64 md:h-80">
                  ...
                  <div className="h-full bg-secondary/10 flex items-center justify-center p-8 text-center">
                    <div>
                      ...
                      <p className="text-sm font-medium text-gray-500">{t("service.satisfaction", "Client Satisfaction Guaranteed")}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="lg:col-span-1 space-y-6 relative z-10">
              ...
              <TiltCard className="bg-primary text-white rounded-3xl p-8 relative overflow-hidden">
                ...
                <h3 className="text-2xl font-bold mb-4">{t("service.ready_grow", "Ready to grow?")}</h3>
                <p className="text-gray-300 mb-8 text-sm">{t("service.ready_grow_desc", "Get a custom strategy tailored to your business needs today.")}</p>
              ...
              <TiltCard className="bg-white dark:bg-gray-900 rounded-3xl p-8 border border-gray-100 dark:border-gray-800 shadow-sm">
                <h3 className="text-lg font-bold text-primary dark:text-white mb-2">{t("service.need_help", "Need immediate help?")}</h3>
                <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">{t("service.call_us", "Call our support team 24/7")}</p>
